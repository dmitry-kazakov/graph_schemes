...
package body Test_ODBC_Session is
   ...
   function Open return Storage_Handle is
      Erase : constant Boolean := First_Time;
   begin
      if First_Time then
         Query;
      end if;
      loop
         ...
         begin
            Result :=
               Create
               (  To_UTF8 (To_String (Server_Name)),
                  To_UTF8 (To_String (User_Name)),
                  To_UTF8 (To_String (Password)),
                  Erase
               );
            if Length (Trace_File) /= 0 then
               Persistent.Native_ODBC.Enable_Tracing
               (  Result,
