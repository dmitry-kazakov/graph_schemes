...
package body ODBC.API.Lectures is
   ...
   begin
      ...
      Bind_Result (Command.all, 1, Count'Unchecked_Access);
      Execute (Command.all);
      Fetch (Command.all);
      Close_Cursor (Command.all);
      return Natural (Count);
   exception
      when End_Error | Status_Error =>
         Close_Cursor (Command.all);
         return 0;
      when Data_Error =>
         Close_Cursor (Command.all);
   ...
   function Get_Features_No
            (  Command    : not null access ODBC_Command'Class;
               Table_Name : String
            )  return Natural is
      Count : aliased SQLINTEGER;
   begin
   ...
   begin
      ...
      case Code is
         ...
         when SQL_NO_DATA =>
            ...
            return False;
         when others =>
            Check (Command.all, Code, No_Filter'Access);
            return False;
      end case;
   exception
      when End_Error | Status_Error =>
   ...
   procedure Put_Checked
      ...
      Data : aliased String_Parameter := Create (Value);
   begin
      if Update then
         Prepare
         (  Command,
